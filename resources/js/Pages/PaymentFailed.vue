  ...
  <div class="min-h-screen" :class="{ 'rtl': store.isRTL }">
    ...
    <main class="container mx-auto px-4 py-24 text-center">
      <div class="max-w-2xl mx-auto bg-card border border-border p-12 rounded-3xl shadow-2xl">
        <!-- Error icon -->
        <div class="w-20 h-20 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-8 shadow-inner animate-bounce">
          <svg class="h-10 w-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12" />
          </svg>
        </div>

        <h1 class="text-4xl font-extrabold mb-4 text-red-600">{{ store.t('paymentFailed') }}</h1>
        <p class="text-lg text-muted-foreground mb-10">{{ store.t('paymentFailedMessage') }}</p>

        <!-- Order Details in Amber/Red theme -->
        <div class="bg-red-50/50 border border-red-100 rounded-2xl p-6 mb-10 grid grid-cols-2 gap-4 rtl:text-right">
          <div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
          <a :href="route('checkout')" class="bg-red-600 text-white px-10 py-4 rounded-xl font-bold hover:bg-red-700 hover:shadow-xl transition-all-300 hover:-translate-y-1">
            {{ store.t('tryAgain') }}
          </a>
          <a :href="route('homepage')" class="bg-secondary text-white px-10 py-4 rounded-xl font-bold hover:bg-muted hover:text-gray-400 transition-all-300 hover:-translate-y-1">
            {{ store.t('backToHome') }}
          </a>
        </div>
      </div>
