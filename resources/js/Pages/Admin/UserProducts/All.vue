              ...
              <tr v-for="item in userProducts.data" :key="item.id" class="font-sans-latin text-sm font-medium">
                  <td class="mx-auto max-w-sm p-2 text-sm leading-6 sm:text-base sm:leading-7">
                      <div class="flex items-center flex-wrap">
                          ...
                          <div class="ml-1">
                              <div class="text-sm font-medium">{{ item.product_color.product_name }} - {{ item.size }} - {{ item.barcode }} </div>
                          </div>
                      </div>
                  </td>
                  ...
                  <td class="mx-auto max-w-sm p-6 text-sm leading-6 sm:text-base sm:leading-7">
                      <div class="ml-4">
                          <div class="text-sm text-center font-medium bg-fuchsia-200 p-1 rounded-lg">Barcode: {{ item.product_color.barcode }}</div>
      ...
      watch: {
          params: {
              handler: throttle(function () {
                  ...
                  //axios.get(app_url+'userProducts/all/products', this.params).then(response => {
                      this.userProducts = {
                          ...response.data,
                          data: [...response.data.data]
                      }
                  });
                  // this.$inertia.get(this.route('userProducts.all'), this.params, { replace: true, preserveState: true});
              }),
              deep: true
          }
      },
