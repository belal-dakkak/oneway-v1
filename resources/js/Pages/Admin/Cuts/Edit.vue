            ...
            <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
                <form class="w-full" @submit.prevent v-on:submit.prevent>
                    ...
                    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8 flex justify-around">
                        ...
                        <div class="w-full px-6">
                            <jet-label for="date" :value="'تاريخ القصة: '+date" dir="rtl" />
                    ...
                    <div class="flex justify-between">
                        ...
                        <div>
                            <img :src="cut.image_url" class="rounded-md w-full h-fit">
                        </div>
                    </div>
                    <jet-section-border />

                    <jet-button :type="'button'" @click="updateCutSimple">
                        حفظ
                    </jet-button>
                </form>
            </div>
...
export default defineComponent({
    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                size: this.cut.size,
                name: this.cut.name,
                total: this.cut.total,
                date: this.date,
                image: this.cut.image,
                sizesOptions: [],
                sizes: this.sizes,
                colorsOptions: [],
                colors: this.colors,
    ...
    methods: {
        updateCutSimple() {
            ...
            this.form.post(route('cuts.update', this.cut.id), {
                errorBag: 'updateCutSimple',
                preserveScroll: true
            });
        },
        addSize (newTag) {
