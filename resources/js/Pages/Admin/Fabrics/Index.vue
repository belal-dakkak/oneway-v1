  ...
  <app-layout title="Products Management">
      <div class="flex justify-around mt-8">
          <div class="flex justify-between">
              <inertia-link :href="route('fabrics.create')">
          ...
          <div class="mt-8 flex justify-between space-x-4">
              ...
              <h3 class="text-2xl font-bold capitalize text-primary">الأقمشة</h3>
          </div>
      </div>
    <MeeTable :tableTitle="'All Products'">
        <div v-if="fabrics.data.length === 0" class="my-40 flex items-center justify-center text-xl font-bold text-error">
            <span class="text-center">لا يوجد أقمشة!</span>
        </div>
        <table v-else class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200 text-pcr">
            <tr v-for="item in fabrics.data" :key="item.id" class="font-sans-latin text-sm font-medium">
                ...
                <td class="mx-auto max-w-sm p-6 text-sm leading-6 sm:text-base sm:leading-7">
                    <div class="ml-4">
                        <div class="text-sm font-medium" dir="rtl">{{ item.yards }}</div>
                    </div>
                </td>
                <td class="mx-auto max-w-sm p-6 text-sm leading-6 sm:text-base sm:leading-7">
                    <div class="ml-4">
                ...
                <td class="fle justify-between mx-auto max-w-sm p-6 text-sm leading-6 sm:text-base sm:leading-7">
                    <inertia-link :href="route('fabrics.edit', item.id)" class="p-2 m-1 pt-4 rounded-md text-white btn-ghost bg-teal-400 hover:bg-teal-600 hover:text-white">
                        <vue-feather :type="'edit'" stroke-width="2"></vue-feather>
                    </inertia-link>
                    <inertia-link :href="route('fabrics.logs', item.id)" class="p-2 m-1 pt-4 rounded-md text-white btn-ghost bg-teal-400 hover:bg-teal-600 hover:text-white">
        ...
        <pagination class="mt-10" :links="fabrics.links"></pagination>
    </MeeTable>
  </app-layout>
</template>
...
export default {
    ...
    methods: {
        ...
        async exportFabric(id) {
            await this.$swal({
                title: 'تخريج دفعة من القماش',
                text: "رجاءً أدخل قيمة الياردات",
                input: 'text',
                inputPlaceholder: 'قيمة الياردات',
                showCancelButton: true,
                confirmButtonColor: '#014758',
                cancelButtonColor: '#d33',
                confirmButtonText: 'تخريج الياردات',
                width: 400,
                padding: '1em',
                color: '#014758',
                inputValidator: (value) => {
                    return new Promise((resolve) => {
                        if (value > 0) {
                            ...
                            resolve('يجب أن تدخل قيمة الياردات')
                        }
                    })
                }
            }).then((result) => {
                if (result.isConfirmed && result.value) {
                    axios.post(route('fabrics.exports', {'id': id}), {
                        id: id,
                        ...
                        if (response.data.success){
                            ...
                            this.$inertia.get(this.route('fabrics.index'))
                        } else
    ...
    watch: {
        params: {
            handler: throttle(function () {
                ...
                this.$inertia.get(this.route('fabrics.index'), this.params, { replace: true, preserveState: true});
            }),
            deep: true
        }
    },
