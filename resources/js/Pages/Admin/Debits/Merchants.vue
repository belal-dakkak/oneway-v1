<template>
  <app-layout title="Products Management">
      <div class="mt-8 flex justify-around space-x-4">
          ...
          <h3 class="text-2xl mt-4 font-bold capitalize text-primary"><span class="bg-emerald-500 px-2 py-0 text-6xl rounded-md text-white">{{getAllDebits()}} </span> ديون للتجار</h3>
          <h3 class="text-2xl mt-4 font-bold capitalize text-primary"><span class="bg-teal-500 px-2 py-0 text-6xl rounded-md text-white">{{belal_debit}} </span> ديون بلال</h3>
          <div class="max-w-xs">
      ...
      <MeeTable :tableTitle="'All Products'">
        <div v-if="debitMerchants.data.length === 0 && belal_merchant.length === 0" class="my-40 flex items-center justify-center text-xl font-bold text-error">
        ...
        <table v-else class="min-w-full divide-y divide-gray-200">
          ...
          <tr v-for="item in belal_merchant" :key="item.id" class="font-sans-latin text-sm font-medium">
              ...
              <td class="flex flex-wrap mx-auto max-w-sm p-6 text-sm leading-6 sm:text-base sm:leading-7">
                  ...
                  <div class="m-4">
                      <button @click="addPayment(item.id)" class="w-32 flex justify-between rounded-md text-white bg-teal-400 hover:bg-teal-600 hover:text-white">
                  ...
                  <div class="m-4">
                      <inertia-link :href="route('debits.merchants.show', item.id)" class="w-32 flex justify-between rounded-md text-white bg-fuchsia-600 hover:bg-fuchsia-800 hover:text-white">
                          <p class="m-1 p-1">الدفعات</p>
                          <vue-feather class="m-2 mt-3" :type="'align-center'" stroke-width="2"></vue-feather>
                      </inertia-link>
                  </div>

                  <div class="m-4" v-if="false">
                      <inertia-link :href="route('debits.index', {creditor: item.creditor_id, debtor: item.debtor_id})" class="w-32 flex justify-between rounded-md text-white bg-indigo-600 hover:bg-indigo-800 hover:text-white">
                  ...
                  <div class="m-4">
                      <inertia-link :href="route('debits.log', item.id)" class="w-32 flex justify-between rounded-md text-white bg-indigo-600 hover:bg-indigo-800 hover:text-white">
                          <p class="m-1 p-1">التفاصيل</p>
                          <vue-feather class="m-2 mt-3" :type="'align-center'" stroke-width="2"></vue-feather>
                      </inertia-link>
                  </div>
                  ...
                  <div class="m-4">
                      <button @click="refundProduct(item)" class="w-32 flex justify-between rounded-md text-white bg-rose-400 hover:bg-red-600 hover:text-white">
          ...
          <tr v-for="item in debitMerchants.data" :key="item.id" class="font-sans-latin text-sm font-medium">
                <td class="mx-auto max-w-sm p-6 text-sm leading-6 sm:text-base sm:leading-7">
                    <div class="ml-4">
                        <div class="text-sm font-medium">{{ item.creditor.name }}</div>
                    </div>
                  ...
                  <div class="modal-wrapper m-24 ">
                      <div class="modal-container relative">
                          ...
                          <div class="modal-header flex justify-center">
                              <h3 class="text-teal-600 text-3xl font-bold">مرتجع للتاجر {{item.creditor.name}}</h3>
                          </div>
                            ...
                            <div class="px-4" dir="rtl">
                                ...
                                <div class="flex justify-around items-stretch" v-for="(rproduct, index) in rproducts" style="padding: 20px;border: 1px solid;">
                                    ...
                                    <div class="px-1 w-1/4">
                                        ...
                                        <jet-label v-if="rproduct" for="qty" :value="' العدد المسموح '+rproduct.stock" dir="rtl" />
                                        <jet-input :id="rproduct+'stock'" type="number" class="mt-1 block w-full" v-model="rproduct.stock" autocomplete="stock" />
                                    </div>
...
export default {
    ...
    props: {
        ...
        belal_debit: Number,
        rate: Number,
        belal_merchant: Array,
        filters: Object,
        shops: Array
    },
    data() {
        var debitMerchantsd = []
        this.merchants.data.forEach(item => {
            debitMerchantsd.push({
    ...
    methods: {
        controlProduct(event){
            ...
            if ((event.keyCode >= 48 && event.keyCode <= 57) || (event.keyCode >= 65 && event.keyCode <= 90) || (event.keyCode >= 95 && event.keyCode <= 105)){
                if(model.length > 4){
                    axios.get(this.route('userProducts.merchant', data)).then(response => {
                        for (let index = 0; index < response.data.length; index++) {
                            let found = this.rproducts.find(x => x.barcode === response.data[index].barcode)
                            if(found){
        ...
        deleteProduct(index){
            if (this.rproducts.length > 1 && this.rproducts[index].barcode)
        ...
        confirmResult(){
            ...
            if (!this.rproducts ){
                ...
                // formData.append('user_product_id', this.product.id)
                let prods = []
                for (let index = 0; index < this.rproducts.length; index++) {
                    prods.push({'qty':this.rproducts[index].stock,'id':this.rproducts[index].id})
                }
                formData.append('user_products', JSON.stringify(prods))
                formData.append('merchant_id', this.item.creditor_id)
                formData.append('merchant_debit_id', this.item.id)
                axios.post(this.route('merchantRefunds.store'), formData,{
                    headers: {
                    ...
                    if (result.data.success){
                        ...
                        this.$inertia.get(route('debits.merchants'));
                    }else{
        ...
        async refundProduct(item) {
            ...
            this.merchant_id = item.creditor_id
            let data = {
              'user_id': item.debtor_id,
              'merchant_id': item.creditor_id
            };
            this.item = item;
            this.showModal = true;
            // axios.get(this.route('userProducts.merchant', data)).then(response => {
                ...
                if (result.isConfirmed && result.value) {
                    axios.post(route('debits.payment'), {
                        ...
                        if (response.data.success){
                            ...
                            this.$inertia.get(this.route('debits.merchants'))
                        } else
        ...
        payDebit(id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    ...
                    this.$inertia.get(route('merchants.pay', id))
                }
            })
    ...
    watch: {
        params: {
            handler: throttle(function () {
                ...
                axios.get(this.route('debits.merchants', this.params)).then(response => {
                    this.debitMerchants = {
                        ...response.data,
                ...
                // this.$inertia.get(this.route('debits.merchants'), this.params, { replace: true, preserveState: true});
            }),
            deep: true
        },
        stock: {
