        ...
        <div>
            <div class="max-w-full px-10 mx-auto py-5 sm:px-6 lg:px-8">
                <form class="w-full" @submit.prevent v-on:submit.prevent>
                    <tax-price-notice order-type="complex_from_multi" :tax-ratio="tax_ratio" :enabled="enable_tax" :currency="form.currency" />
                ...
                <sales-cashbox-card :title="__('Sales Fund')" />
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            workingProduct: null,
            products: [{barcode: '', qty: '', price: '', product_id: '', name: '', image: '', qty_limit: '',size:''}],
            isDeleting: false
        }
    },
            ...
            this.form.post(route('orders.store'), {
                ...
                onSuccess: (res) => {
                    if (res.props.flash.server_error)
                        ...
                        this.products = [{barcode: '', qty: '', price: '', product_id: '', name: '', image: '', qty_limit: '',size:''}];
            ...
            if ((event.keyCode >= 48 && event.keyCode <= 57) || (event.keyCode >= 65 && event.keyCode <= 90) || (event.keyCode >= 95 && event.keyCode <= 105)){
                if (found.qty != 0 && found.qty != '' && found.qty != undefined){
                    if (found.qty < found.qty_limit){
                        ...
                        element.value = ''
                        this.products.pop()
                        this.products.push({ barcode: null, qty: '', price: '', product_id: '', name: '', image: '', qty_limit: '',size:'' })
                    ...
                    axios.post(this.route('orders.multi.match'), formData)
                        .then((response) => {
                            if (response.data){
                                for(let i = 0; i < response.data.length; i++){
                                    let newBarcode = response.data[i].barcode;

                                    let found = this.products.find(x => x.barcode === newBarcode)
                                    let cond = false
                                    if(found)
                                        cond = found.qty != 0 && found.qty != '' && found.qty != undefined
                                    if(cond){
                                        if (found.qty < found.qty_limit){
                                            found.qty ++
                                        ...
                                        let sale_price = response.data[i] && response.data[i].product_color && response.data[i].product_color.product ? response.data[i].product_color.product.sale_price : 0;

                                        let [lastItem] = this.products.slice(-1)
                                        //lastItem.price = Currency.exchange(response.data[i].retail_price, this.rate)
                                        lastItem.price = Currency.exchange(sale_price, this.rate)
                                        lastItem.qty = 1
                                        lastItem.size = response.data[i].size
                                        lastItem.barcode = response.data[i].barcode;
                                        lastItem.qty_limit = response.data[i].stock
                                        lastItem.product_id = response.data[i].id
                                        lastItem.image = response.data[i].product_color.photo_url
                                        lastItem.name = response.data[i].product_color.product_name+" - "+response.data.size
                                        this.products.push({ barcode: '', qty: '', price: '', product_id: '', name: '', image: '', qty_limit: '',size:'' });
                                    }
                                }
        ...
        products: {
            handler: throttle(function () {
                ...
                if(enable_tax == 'yes') {
                    ...
                    var order_total_price_without_tax = totalPrice;
                    var order_total_tax_value       = (order_total_price_without_tax * tax_ratio) / 100;
                    var order_total_price         = order_total_price_without_tax + order_total_tax_value;
                    ...
                    this.form.total_price = Number(order_total_price) - Number(this.form.discount);
                    ...
                    this.form.total_qty = totalQty;
                    this.form.paid_price = totalPrice;
                }
