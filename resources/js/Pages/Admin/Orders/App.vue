        ...
        <table v-else class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="item in userOrders.data" :key="item.id" class="font-sans-latin text-sm font-medium">
                ...
                <td class="mx-auto max-w-sm p-6 text-sm leading-6 sm:text-base sm:leading-7">
                    <div class="ml-4">
                        <button @click="changeStatus(item)" v-if="item.status === 1" class="bg-rose-400 rounded-md px-4 py-2">Pending</button>
                    </div>
                    <div class="ml-4">
                        <button @click="changeStatus(item)" v-if="item.status === 2" class="bg-yellow-400 rounded-md px-4 py-2">Ongoing</button>
                    </div>
                    <div class="ml-4">
                        <button v-if="item.status === 3" class="bg-green-400 rounded-md px-4 py-2">Delivered</button>
                    </div>
                </td>
                          ...
                          <div class="modal-body border-b-4 border-state-500 pb-6">
                              <div class="flex grid grid-cols-2 items-stretch">
                                  <div class="flex justify-between p-2" v-for="product in products">
                                      <img class="w-20 h-20 rounded-md" :src="product.product.photo_url" alt="">
                                      <div class="text-sm mx-1 text-left" dir="rtl">{{ product.product.product_name }} ( {{ product.qty }} )</div>
                                  </div>
                              </div>
                          </div>
                      </div>
...
export default {
    ...
    methods: {
        async showProducts(item) {
            this.showModal = true;
            this.item = item;
            this.products = item.product_items;
        },
        closeModal(){
        ...
        changeStatus(item){
            ...
            axios.post(this.route('orders.change', item.id), formData)
                .then((response) => {
                    this.isLoading = false;
                    this.showSuccessMessage('تمت عملية الطباعة بنجاح')
                ...
                if (result.isConfirmed && result.value) {
                    axios.post(route('orders.payment'), {
                        ...
                        if (response.data.success){
                            ...
                            this.$inertia.get(this.route('orders.appOrders'))
                        } else
        ...
        handleFilter(params){
            ...
            axios.get(this.route('orders.appOrders', this.params)).then(response => {
