<template>
  <app-layout title="Mobile Slider Management">
      <div class="flex justify-around mt-8">
          <div class="flex justify-between">
              <inertia-link :href="route('mobile-sliders.create')">
                  <jet-button class="px-16 mt-8 bg-pcr float-right">
                      {{ __('Add Mobile Slider')}}
                  </jet-button>
              </inertia-link>
          </div>
      </div>
    <MeeTable :tableTitle="'All Mobile Sliders'">
        <div v-if="sliders.data.length === 0" class="my-40 flex items-center justify-center text-xl font-bold text-error">
            <span class="text-center">{{ __('No Mobile Sliders')}}</span>
        </div>
          ...
          <tbody class="bg-white divide-y divide-gray-200 text-pcr">
            ...
            <tr v-for="slider in sliders.data" :key="slider.id" class="font-sans-latin text-sm font-medium">
                ...
                <td class="mx-auto max-w-sm p-2 text-sm leading-6 sm:text-base sm:leading-7">
                    <div class="flex items-center">
                        <inertia-link :href="route('mobile-sliders.edit', slider.id)" class="p-2 m-1 pt-4 rounded-md text-white btn-ghost bg-teal-400 hover:bg-teal-600 hover:text-white">
    ...
    methods: {
        ...
        deleteSlider(id) {
            this.$swal({
                ...
                confirmButtonText: 'نعم احذف سلايدر الموبايل',
                width: 400,
                padding: '1em',
                color: '#014758',
            }).then((result) => {
                if (result.isConfirmed) {
                    this.$swal.fire({
                        html: '<p class="text-white pt-5 font-extrabold">تم الحذف بنجاح</p>',
                        icon: 'success',
                        iconColor: '#FFFFFF',
                        width: 400,
                    ...
                    this.$inertia.delete(route('mobile-sliders.destroy', id))
                }
            })
    ...
    watch: {
        params: {
            handler: throttle(function () {
                ...
                this.$inertia.get(this.route('mobile-sliders.index'), this.params, { replace: true, preserveState: true});
            }, 150),
            deep: true
        }
    }
